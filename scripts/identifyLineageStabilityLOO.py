...
class IdentifySufficientLineages(object):
    ...
    def run(self, ubiquityThreshold, singleCopyThreshold, minGenomes, minMarkers, mostSpecificRank):
        img = IMG()

        lineages = img.lineagesByCriteria(minGenomes, mostSpecificRank)

        fout = open('./data/lineage_evaluation.tsv', 'w')
        fout.write('Lineage\t# genomes\t# markers\tmean\tstd\tmean %\tmean + std %\tmean + 2*std %\n')
        ...
        for lineage in lineages:
            ...
            fout.write(lineage + '\t' + str(len(genomeIds)) + '\t' + str(len(markerGenes)))

            # withhold select percentage of genomes and calculate new marker set
            changeMarkerSetSize = []
            for genomeId in genomeIds:
                subsetGenomeIds = genomeIds.difference([genomeId])
...
if __name__ == '__main__':
    ...
    identifySufficientLineages.run(args.ubiquity, args.single_copy, args.min_genomes, args.min_markers, args.most_specific_rank)
