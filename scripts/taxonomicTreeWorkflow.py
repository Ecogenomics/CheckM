...
class GenomeTreeWorkflow(object):
    ...
    def __taxonomicMarkers(self, taxaStr, metadata, ubiquityThreshold, singleCopyThreshold):
        ...
        genomeIds = self. img.genomeIdsByTaxonomy(taxaStr, metadata, 'trusted')
    ...
    def inferGeneTrees(self, phyloUbiquityThreshold, phyloSingleCopyThreshold, numThreads, outputGeneDir, outputModelDir):
        ...
        gammaGenomeIds, gammaMarkers = self.__taxonomicMarkers('Bacteria;Proteobacteria;Gammaproteobacteria', metadata, phyloUbiquityThreshold, phyloSingleCopyThreshold)
        betaGenomeIds, _ = self.__taxonomicMarkers('Bacteria;Proteobacteria;Betaproteobacteria', metadata, phyloUbiquityThreshold, phyloSingleCopyThreshold)
        ...
        # infer tree for the Gammas and Beta using the Alphas as an outgroup
        genomeIds = gammaGenomeIds.union(betaGenomeIds) #.union(alphaGenomeIds)
        print '  Identified genomes: %d' % len(genomeIds)
        
        self.imgIdsToAceIds(genomeIds)
        
        # use the Gamma + Beta marker genes to obtain high resolution within this taxonomic group
        markerGenes = gammaMarkers #.intersection(betaMarkers)
        print '  Identified markers: %d' % len(markerGenes)
        ...
        self.__fetchMarkerModels(markerGenes, outputModelDir)

        # align gene sequences and infer gene trees
        print '  Aligning marker genes:'
        self.__alignMarkers(genomeIds, markerGenes, genesInGenomes, numThreads, outputGeneDir, outputModelDir)
    ...
    def run(self, numThreads):  
        ...
        genomeIds = self.inferGeneTrees(self.phyloUbiquity, self.phyloSingleCopy, numThreads, self.alignmentDir, self.hmmDir)
