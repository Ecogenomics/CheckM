...
class GenomeCharacteristics(object):
    ...
    def parseGFF(self, genomeId):
        ...
        for codon, count in list(startCodonFreq.items()):
            startCodonFreq[codon] = float( count ) / max(genes, 1)
        for codon, count in list(stopCodonFreq.items()):
            stopCodonFreq[codon] = float( count ) / max(genes, 1)
        for motif, count in list(rbsMotifFreq.items()):
        ...
        for codon, count in list(startCodonFreqPlasmid.items()):
            startCodonFreqPlasmid[codon] = float( count ) / max(genesPlasmid, 1)
        for codon, count in list(stopCodonFreqPlasmid.items()):
            stopCodonFreqPlasmid[codon] = float( count ) / max(genesPlasmid, 1)
        for motif, count in list(rbsMotifFreqPlasmid.items()):
    ...
    def writeResults(self, charByGroup):
        ...
        for groupName, data in list(charByGroup.items()):
            #sanity check
    ...
    def run(self):
        ...
        for domain, count in list(missingGenomeData.items()):
            print(('  ' + domain + ': ' + str(count)))

        # process all genomes
        charByGroup = {}
        for genomeId, groupName in list(genomeIdToGroup.items()):
            print(genomeId)
