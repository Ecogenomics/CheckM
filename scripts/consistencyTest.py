...
class ConsistencyTest(object):
	...
	def run(self, geneTreeDir, treeExtension, consistencyThreshold, minTaxaForAverage, outputFile, outputDir):			
		...
		# get TIGRFam info
		descDict = {}
		files = os.listdir('./data/TIGRFAMs_13.0_INFO')
		for f in files:
			shortDesc = longDesc = ''
			for line in open('./data/TIGRFAMs_13.0_INFO/' + f):
		...
		# get PFam info
		for line in open('./data/Pfam-A.clans.tsv'):
			lineSplit = line.split('\t')
			acc = lineSplit[0]
		...
		for treeFile in treeFiles:
			...
			# write results
			fout = open(os.path.join('./data/consistency_index', treeFile + '.results.tsv'), 'w')
			fout.write('Tree')
			for r in xrange(0, 3):
