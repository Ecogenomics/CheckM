...
class BinComparer(object):
    ...
    def __binningStats(self, bins, seqLens):
        ...
        processedSeqs = set()
        repeats = set()
        for binId, seqs in list(bins.items()):
            numBinnedBases = 0
            for seqId in seqs:
    ...
    def report(self, binFiles1, binFiles2, seqFile, outputFile):
        ...
        # sort bins by size
        binStats1 = sorted(iter(list(binStats1.items())), key=lambda x: x[1][1], reverse=True)
        binStats2 = sorted(iter(list(binStats2.items())), key=lambda x: x[1][1], reverse=True)
